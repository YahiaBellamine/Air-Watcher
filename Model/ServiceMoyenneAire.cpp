...
static float *moyenneAire(float centre_lat, float centre_long, float rayon, list<SerieMesures> *toutesLesMesures, list<Capteur> *tousLesCapteurs, Temps dateDebutMesures)
{
    ...
    else
    {
        ...
        {
            resultat[0] += (*it).getMesure("O3").getValeur();
            resultat[1] += (*it).getMesure("NO2").getValeur();
            resultat[2] += (*it).getMesure("SO2").getValeur();
